...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, transform=None,valid=False):
        ...
        self.valid=valid
        if valid:
            self.root='Datasets/AugmentedDataset_new/images/val'
            self.names=os.listdir(self.root)
        else:
            self.root='Datasets/AugmentedDataset_new/images/train'
            self.names=os.listdir(self.root)
    ...
    def __getitem__(self, idx):
        ...
        label = cv2.imread(image_name.replace("images","labels").replace("jpg","png"), 0)
        ...
        _,seg_b1 = cv2.threshold(label,1,255,cv2.THRESH_BINARY_INV)
        _,seg1 = cv2.threshold(label,1,255,cv2.THRESH_BINARY)
